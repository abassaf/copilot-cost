...
- **Billable spans only:** `isChatSpan()` must accept leaf `chat` completions and reject parent `invoke_agent` rollups (and `execute_tool`). Those parents copy the sum of child `gen_ai.usage.*` / `github.copilot.nano_aiu`; counting both doubles dashboard totals vs the Copilot CLI usage panel. See `docs/otel-span-accounting.md`. `input_tokens` on spans is total input (includes cache); parser stores fresh = total − cache_read − cache_creation.
- `src/import/` — history backfill. `session-state.ts` scans `~/.copilot/session-state/*/events.jsonl`, pulls the per-model cost basis off `session.shutdown.modelMetrics`, and reconciles it against `assistant.message.outputTokens` before trusting it (a resumed session keeps only its last run's counters). `index.ts` detects the OTel cutover and emits OTel-shaped leaf `chat` spans to `copilot-cost-imported.jsonl`. Imported usage flows through the normal parser/pricing path; nothing downstream special-cases it. See `docs/importing-history.md`.
